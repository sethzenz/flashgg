...
namespace flashgg {
    ...
    public:
        ...
        struct Bin {
            ...
            std::vector<double> err; // length: number of parameters (usually 1 or 2)

            Bin( std::vector<double> mi, std::vector<double> ma, std::vector<double> va, std::vector<double> er) :
                min(mi), max(ma), val(va), err(er) {}
        };

        ObjectSystMethodBinnedByFunctor( const edm::ParameterSet &conf ) : BaseSystMethods<flashgg_object,param_var>::BaseSystMethods(conf) {
        ...
        std::pair<std::vector<double>,std::vector<double> > binContents (const flashgg_object& y) {
            ...
            for (auto bin : bins_) {
                ...
                if (found) {
                    return std::make_pair(bin.val,bin.err);
                }
            }
